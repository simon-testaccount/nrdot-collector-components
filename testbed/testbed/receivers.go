...
// https://github.com/newrelic/nrdot-collector-components/commits/main/testbed/testbed/receivers.go?since=2025-11-26
//
...
func (bor *BaseOTLPDataReceiver) Start(tc consumer.Traces, mc consumer.Metrics, lc consumer.Logs) error {
	...
	if bor.exporterType == "otlp" {
		...
		cfg.HTTP.Get().ServerConfig.NetAddr = confignet.AddrConfig{Endpoint: fmt.Sprintf("127.0.0.1:%d", bor.Port), Transport: confignet.TransportTypeTCP}
	}
